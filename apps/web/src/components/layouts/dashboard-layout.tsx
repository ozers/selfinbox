...
import { Mail, LayoutDashboard, Globe, Inbox, Settings, LogOut, Menu, X, Sun, Moon, ArrowRight } from "lucide-react"
...
export function DashboardLayout() {
  ...
  // When mounted inside /demo/*, sidebar nav must stay inside /demo so links
  // don't kick visitors out to /dashboard (where ProtectedRoute would bounce
  // them to /login).
  const isDemo = useMockEnabled()
  const linkPrefix = isDemo ? "/demo" : ""
  const overviewPath = isDemo ? "/demo" : "/dashboard"
  const inboxPath = `${linkPrefix}/inbox`
  const domainsPath = `${linkPrefix}/domains`
    ...
    <div className="flex min-h-screen bg-background">
      ...
      >
        ...
        <nav className="flex flex-1 flex-col gap-1 p-3">
          {[
            { to: overviewPath, icon: LayoutDashboard, label: "Overview" },
            { to: domainsPath, icon: Globe, label: "Domains" },
          ].map((item) => {
          ...
          <div>
            <div className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium ${
              location.pathname.startsWith(inboxPath) ? "text-primary" : "text-muted-foreground"
            }`}>
            ...
            <div className="ml-4 mt-0.5 flex flex-col gap-0.5 border-l border-border pl-3">
              {/* All Mail */}
              {(() => {
                const isActive = location.pathname === inboxPath && !new URLSearchParams(location.search).get("domain")
                return (
                  <NavLink
                    to={inboxPath}
                    onClick={() => setSidebarOpen(false)}
              ...
              {domains.length > 0 && (
                <>
                  ...
                  {domains.map((d) => {
                    ...
                    const isDomainActive = location.pathname === inboxPath && activeDomain === d.domain && !activeAddress
                    return (
                      <div key={d.id}>
                        <NavLink
                          to={`${inboxPath}?domain=${d.domain}`}
                          onClick={() => setSidebarOpen(false)}
                        ...
                        {d.addresses.filter((a) => a.isActive && !a.isCatchall).map((a) => {
                          const isAddrActive = location.pathname === inboxPath && activeAddress === a.address
                          const prefix = a.address.split("@")[0]
                          return (
                            <NavLink
                              key={a.id}
                              to={`${inboxPath}?address=${a.address}`}
                              onClick={() => setSidebarOpen(false)}
      ...
      <div className="flex min-w-0 flex-1 flex-col lg:ml-60">
        ...
        {isDemo && (
          <div className="border-b border-primary/20 bg-primary/5 px-4 py-2.5 sm:px-6 lg:px-8">
            <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3 text-xs sm:text-sm">
              <span className="text-foreground/80">
                <span className="font-semibold text-primary">Demo</span> — sample data, no real mail. Buttons that send mail won't work here.
              </span>
              <a
                href="https://github.com/ozers/selfinbox"
                target="_blank"
                rel="noreferrer"
