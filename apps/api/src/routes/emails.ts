...
// GET /api/emails/:id/attachments/:idx
//
// Streams a single attachment from S3 with a defense-in-depth header set:
//
//   - Content-Disposition: attachment (always — opt-in to inline only for
//     verified image MIMEs, and never for active content)
//   - X-Content-Type-Options: nosniff (block browser MIME sniffing)
//   - Content-Security-Policy: default-src 'none'; sandbox (HTML/SVG that
//     slips through would still execute in a null origin sandbox)
//   - Referrer-Policy: no-referrer
//   - HTML/SVG content is force-served as text/plain regardless of disposition
//   - RFC 6266 filename* for unicode filenames + ASCII fallback
//
// Owner check happens via the user_id join — no UUID-guessing access.
emails.get("/:id/attachments/:idx", async (c) => {
  const userId = c.get("userId");
  const emailId = c.req.param("id");
  ...
  const [email] = await sql`SELECT attachments_meta FROM emails WHERE id = ${emailId} AND user_id = ${userId}`;
  if (!email) return c.json({ error: "Email not found" }, 404);

  const list: AttachmentMeta[] = Array.isArray(email.attachments_meta)
    ? email.attachments_meta
    : (email.attachments_meta ? JSON.parse(email.attachments_meta) : []);

  const att = list.find((a) => a.idx === idx);
  if (!att) return c.json({ error: "Attachment not found" }, 404);

  // Honor ?inline=1 only if both the stored metadata says it's safe-to-inline
  // and the request asks for it. Anything else falls through to attachment.
  ...
  // Decide the Content-Type we serve under. Active content (HTML/SVG/JS)
  // gets force-downgraded to text/plain so a browser cannot render it even
  // if a determined user strips the Content-Disposition header.
  const isActive = /^(text\/html|image\/svg|application\/xhtml|application\/(x-)?javascript|text\/javascript)/i.test(att.contentType);
  const servedType = isActive ? "text/plain; charset=utf-8" : att.contentType;

  // RFC 6266: ASCII-safe fallback + UTF-8 percent-encoded variant.
  const asciiSafe = att.filename.replace(/[^\x20-\x7e]/g, "_").replace(/["\\]/g, "_");
  const utf8Encoded = encodeURIComponent(att.filename);
  const disposition = `${allowInline ? "inline" : "attachment"}; filename="${asciiSafe}"; filename*=UTF-8''${utf8Encoded}`;

  return new Response(new Uint8Array(body.buffer, body.byteOffset, body.byteLength) as unknown as BodyInit, {
    status: 200,
    headers: {
